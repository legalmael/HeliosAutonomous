...
class WaypointNavigation:
    def __init__(self):
        ...
        rospy.Subscriber('goal', Pose, self.cb_goal)
    ...
    def calculateCtrl(self):
        ...
        if (len(poses) >= 2):
            pt1, pt2 = poses[0].pose, poses[1].pose
    ...
    def cb_pose(self, msg):
        ...
        if len(self.path.poses) >= 2:
            self.path.poses = [self.cur_pose, self.path.poses[1]]

    def cb_goal(self, msg):
        #rospy.loginfo("New goal received : %s", msg)
        self.goal = msg
        if (self.goal == Pose()):
            self.path.poses = []
        else:
            goalStamped = PoseStamped()
            goalStamped.pose = self.goal
            rospy.loginfo("New goal received : %s", self.goal)
            self.path.poses = [self.cur_pose, goalStamped]
